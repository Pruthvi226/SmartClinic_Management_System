...
<c:choose>
    <c:when test="${editMode}">
        <c:url var="patientFormAction" value="/patients/${patient.id}/edit" />
    </c:when>
    <c:otherwise>
        <c:url var="patientFormAction" value="/patients/register" />
    </c:otherwise>
</c:choose>
...
<div class="card" style="max-width: 600px; margin: 0 auto;">
    ...
    <p style="color:var(--text-muted); margin-bottom:2rem;">Fill in the patient's basic information to maintain the medical record.</p>

    <form:form action="${patientFormAction}" method="POST" modelAttribute="patient">
        ...
        <div class="form-group">
            ...
            <form:textarea path="allergies" class="form-control" rows="3" placeholder="e.g. Penicillin, Aspirin. Enter None if no known allergy." />
            <form:errors path="allergies" cssClass="text-danger" />
        </div>

        <div class="form-group">
            <label>Address</label>
            <form:textarea path="address" class="form-control" rows="3" placeholder="Street, city, state" />
        ...
        <div style="margin-top:2rem; display:flex; gap:1rem;">
            <button type="submit" class="btn btn-primary" style="flex:1">
                ...
                ${editMode ? 'Save Patient' : 'Complete Registration'}
            </button>
